const templateFile = await fetch("component/Hero/Hero2/template.html");
const template = await templateFile.text();

const templateFile2 = await fetch("component/Hero/Hero2/template-li.html");
const templateLi = await templateFile2.text();

let Hero2 = {};

Hero2.format = function(obj) {
  let html = template;
  let items = "";
...
Hero2.render = function(where, what) {
  let element = document.querySelector(where);
  let html = "";
  for (let obj of what) {
    html = html + element.innerHTML + Hero2.format(obj);
  }
