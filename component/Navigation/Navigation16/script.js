const templateFile = await fetch("component/Navigation/Navigation16/template.html");
const template = await templateFile.text();

const templateFile2 = await fetch("component/Navigation/Navigation16/template-li.html");
const templateLi = await templateFile2.text();

const templateFile3 = await fetch("component/Navigation/Navigation16/template-button.html");
const templateButton = await templateFile3.text();

let Navigation16 = {};

// Format de l'objet qui génère le HTML dynamique
Navigation16.format = function(obj) {
  let html = template;
...
Navigation16.render = function(where, what) {
  let element = document.querySelector(where);
  let html = "";
  for (let obj of what) {
    html = html + Navigation16.format(obj);
  }
