const templateFile = await fetch("component/Navigation/Navigation15/template.html");
const template = await templateFile.text();

const templateFile2 = await fetch("component/Navigation/Navigation15/template-li.html");
const templateLi = await templateFile2.text();

const templateFile3 = await fetch("component/Navigation/Navigation15/template-button.html");
const templateButton = await templateFile3.text();

let Navigation15 = {};

// Format de l'objet qui génère le HTML dynamique
Navigation15.format = function(obj) {
  let html = template;
...
Navigation15.render = function(where, what) {
  let element = document.querySelector(where);
  let html = "";
  for (let obj of what) {
    html = html + Navigation15.format(obj);
  }
